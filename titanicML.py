...
# ------------------------------------------------------- Phase 1 : Prepare training data, Prepare model
...
### Models can't directly understand text, they work with numbers
...
training_data['Fare'].fillna(training_data['Fare'].median(), inplace=True)

# Define predictors and target --
## Include all relevant features except survived. This ensures the model learns from the input features 'predictors', otherwise it would be just memorizing the outcome (not generalizing)
...
# Get the number of columns in predictors. The model need to know how many features to expect
...
## and the number of columns of features 'must', leaving it open for the number of rows
...
# Compile the model --
## Using 'sgd' as optimizer, and 'categorical_corssentropy' as the loss function
...
# Fit the model --
## Training the model , using 50 epochs , saving 20% of the training data for validation during training,
...
# ------------------------------------------------------- Phase 2 : Prepare test data, Test the model

# Load the test data --
test_data = pd.read_csv('test.csv')

# store passenger id for future implication, because it must be droped
...
## store the result (passenger id, predicted outcome) in a dataframe
...
for lr in lr_to_test:
    ...
    my_optimizer = SGD(learning_rate=float(lr))
